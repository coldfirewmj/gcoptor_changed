...
private:
	...
	inline int checkOccupancy(const Eigen::Vector3d &pos) { return grid_map_.query(pos); }
...
public:
	...
	void initGridMap(voxel_map::VoxelMap& occ_map);

	ASTAR_RET AstarSearch(const double step_size, Eigen::Vector3d start_pt, Eigen::Vector3d end_pt);

	std::vector<Eigen::Vector3d> getPath();
	std::vector<Eigen::Vector3d> Thetastar(std::vector<Eigen::Vector3d> path);
	bool line_of_sight(Eigen::Vector3d &former,Eigen::Vector3d &later);
};
